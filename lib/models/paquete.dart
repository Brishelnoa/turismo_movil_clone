...
class Paquete {
  ...
  final precioTotalServicios = (json['precio_total_servicios'] is Map)
    ...
    Map<String, String?> _extractPriceAndCurrency(Map<String, dynamic> src) {
      ...
      // common direct keys
      final candidates = [
        'price', 'precio', 'precio_base', 'precio_base_bob', 'price_usd', 'precio_usd', 'monto', 'amount', 'total', 'precio_total'
      ];
      ...
      if (amount == null) {
        ...
        for (final key in ['precio_total_servicios', 'precio_total', 'price_info', 'price_details']) {
      ...
      if (currency == null && precioTotalServicios != null) {
        for (final k in ['currency', 'moneda', 'iso_currency']) {
          if (precioTotalServicios.containsKey(k) && precioTotalServicios[k] != null) {
      ...
      if (currency == null) {
        if (src.keys.any((k) => k.toString().toLowerCase().contains('usd'))) currency = 'USD';
        else if (src.keys.any((k) => k.toString().toLowerCase().contains('bob'))) currency = 'BOB';
      }
    ...
    if (json.containsKey('precio_base') && json['precio_base'] != null) {
      parsedPrecioBase = (json['precio_base'] is num)
          ...
          : double.tryParse(json['precio_base'].toString().replaceAll(',', '.')) ?? 0.0;
      ...
      if (p.containsKey('precio_final_usd') && p['precio_final_usd'] != null) {
        parsedPrecioBase = (p['precio_final_usd'] is num)
            ...
            : double.tryParse(p['precio_final_usd'].toString().replaceAll(',', '.')) ?? 0.0;
      } else if (p.containsKey('precio_original_usd') && p['precio_original_usd'] != null) {
        parsedPrecioBase = (p['precio_original_usd'] is num)
            ? (p['precio_original_usd'] as num).toDouble()
            : double.tryParse(p['precio_original_usd'].toString().replaceAll(',', '.')) ?? 0.0;
      } else if (p.containsKey('precio_bob') && p['precio_bob'] != null) {
        parsedPrecioBase = (p['precio_bob'] is num)
            ? (p['precio_bob'] as num).toDouble()
            : double.tryParse(p['precio_bob'].toString().replaceAll(',', '.')) ?? 0.0;
      ...
      parsedPrecioBase = (json['precio_bob'] is num)
          ...
          : double.tryParse(json['precio_bob'].toString().replaceAll(',', '.')) ?? 0.0;
      extractedCurrency = 'BOB';
    } else {
      // Fallback to previous heuristic search
      final r = _extractPriceAndCurrency(json);
      if (r['amount'] != null && r['amount']!.isNotEmpty) {
        parsedPrecioBase = double.tryParse(r['amount']!.replaceAll(',', '.')) ?? 0.0;
      }
    ...
    if (kDebugMode) {
      try {
        debugPrint('[Paquete.fromJson] id=${json['id']} parsedPrecioBase=$parsedPrecioBase (${parsedPrecioBase.runtimeType}) imagen=${(json['imagen_principal'] ?? json['imagen'])}');
      } catch (_) {}
    }
  ...
  precioBase: parsedPrecioBase,
    ...
    moneda: extractedCurrency ?? json['moneda'] as String? ?? json['currency'] as String?,
    );
  }
  ...
  String get displayPrice {
    ...
    try {
      ...
      if (pts != null) {
        ...
        final usdVal = pts['precio_final_usd'] ?? pts['precio_original_usd'] ?? pts['total_usd'];
        if (usdVal != null) {
          if (usdVal is num) {
            final numVal = usdVal.toDouble();
            final text = (numVal % 1 == 0) ? numVal.toInt().toString() : numVal.toStringAsFixed(2);
            return '\$ $text';
          }
    ...
    try {
      ...
      final text = (parsed % 1 == 0) ? parsed.toInt().toString() : parsed.toStringAsFixed(2);
      return '\$ $text';
    } catch (_) {
  ...
  String get displayPriceBs {
    try {
      final pts = precioTotalServicios;
      if (pts != null && pts.containsKey('precio_bob') && pts['precio_bob'] != null) {
    ...
    try {
      ...
      final text = (converted % 1 == 0) ? converted.toInt().toString() : converted.toStringAsFixed(2);
      return 'Bs. $text';
    } catch (_) {
